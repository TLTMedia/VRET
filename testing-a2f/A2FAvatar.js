...
export class A2FAvatar {
  ...
  async _loadVRM(vrmPath) {
    if (typeof BABYLON.VRMFileLoader === 'undefined' && !window._vrmLoaderLoaded) {
      await new Promise((resolve, reject) => {
        const s = document.createElement('script');
        s.src = 'https://xuhuisheng.github.io/babylonjs-vrm/babylon-vrm-loader.js';
        s.onload = () => { window._vrmLoaderLoaded = true; resolve(); };
        s.onerror = reject;
        document.head.appendChild(s);
      });
    }

    await BABYLON.ImportMeshAsync(vrmPath, this.scene);
